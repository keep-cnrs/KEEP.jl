...
if false
        ...
        function solve_trajectory_with_jacobian(u0, tf, vbp)
            ...
            u0_dual = SVector{N}(
                ForwardDiff.Dual{typeof(tag)}(
                    u0[i], ForwardDiff.Partials(ntuple(j -> j == i ? one(T) : zero(T), N))
                ) for i in 1:N
            )
        ...
        function solve_trajectory_with_jacobian_4d(u0, tf, vbp)
            ...
            u0_dual = SVector{5}(
                if i <= N_diff
                    ForwardDiff.Dual{typeof(tag)}(
                    u0[i],
                    ForwardDiff.Partials(ntuple(j -> j == i ? one(T) : zero(T), N_diff)),
                )
                else
                    ForwardDiff.Dual{typeof(tag)}(
                    u0[i], ForwardDiff.Partials(ntuple(j -> zero(T), N_diff))
                )
                end for i in 1:5
            )
    ...
    function shoot_multiple_N!(res, y, N)
        ...
        for i in 1:N
            ...
            # Integrate segment
            x_next_flow, p_next_flow, pv_segment = flow(
                t_start, x_curr, p_curr, t_end, [tf, λ...]; augment=true
            )
            pv_total .+= pv_segment
    ...
    y_guess_multN = generate_initial_guess_N(
        x_direct, p_direct, tf_direct, λ_direct, N_segments
    )
    ...
    function shoot_multiple_N!(res, y, N)
        ...
        for i in 1:N
            ...
            x_flow, p_flow, pv_segment = flow(
                ts[i], x_curr, p_curr, ts[i + 1], [tf, λ...]; augment=true
            )
            pv_total .+= pv_segment
    ...
    for i in 1:N_segments
        ...
        for t in ts_seg
            xt, pt, _ = flow(
                ts_nodes[i], x_node, p_node, t, [tf_sol, λ_sol...]; augment=true
            )
            push!(t_plot, t)
            push!(x_list, xt)
            push!(p_list, pt)
...
function MultipleShooting(flow, boundary_conds!, x_proto, p_proto, λ_proto; kwargs...)
    return MultipleShooting(
        flow, boundary_conds!, length(x_proto), length(p_proto), length(λ_proto), kwargs
    )
end
...
function (ms::MultipleShooting)(res, y, N)
    ...
    for i in 1:N
        ...
        flow_params = [tf; λ]
        x_flow, p_flow, pv_segment = ms.flow(
            ts[i], x_curr, p_curr, ts[i + 1], flow_params; ms.kwargs...
        )
        pv_total .+= pv_segment
...
function generate_initial_guess(
    ms::MultipleShooting, x_direct, p_direct, tf_direct, λ_direct, N
)
    ts = range(0, tf_direct; length=N + 1)[1:N]
    nodes_guess = [vcat(x_direct(t), p_direct(t)) for t in ts]
    return vcat(nodes_guess..., tf_direct, λ_direct)
...
function reconstruct_trajectory(ms::MultipleShooting, y_sol, N; points_per_segment=50)
    ...
    for i in 1:N
        ...
        for t in ts_seg
            xt, pt, _ = ms.flow(
                ts_nodes[i], x_node, p_node, t, [tf_sol; λ_sol]; ms.kwargs...
            )
            push!(t_plot, t)
            push!(x_list, xt)
            push!(p_list, pt)
