...
function continuation_side(ds)
    return continuation(
        prob, PALC(), @set optc.ds = ds; plot=false, verbosity=1, normC=norminf
    )
end
...
function sheet_jumps(br; dsmax=0.1, factor=1.5)
    return [i for i in 2:length(br) if abs(br.sol[i].p - br.sol[i - 1].p) > factor * dsmax]
end

br_fwd = @time continuation_side(+0.01)
br_bwd = @time continuation_side(-0.01)
println(
    "collocation sheet-jumps: ds>0 at indices ",
    sheet_jumps(br_fwd),
    ", ds<0 at indices ",
    sheet_jumps(br_bwd),
)
...
function continuation_side_ms(ds)
    return continuation(
        prob_ms, PALC(), @set optc_ms.ds = ds; plot=true, verbosity=1, normC=norminf
    )
end
...
println(
    "shooting sheet-jumps: ds>0 at indices ",
    sheet_jumps(br_ms_fwd),
    ", ds<0 at indices ",
    sheet_jumps(br_ms_bwd),
)
...
println(
    "Collocation branches: ds>0 ", length(br_fwd), " pts, ds<0 ", length(br_bwd), " pts"
)
println(
    "Shooting branches:    ds>0 ",
    length(br_ms_fwd),
    " pts, ds<0 ",
    length(br_ms_bwd),
    " pts",
