...
println(
    "  tf monotone-decreasing over [8.8, 10.2] ? ",
    issorted(tf_up; rev=true) && issorted(tf_dn),
)
println(
    "  max nontrivial |mu| over the band = ",
...
for vr in 9.0:0.25:10.0
    ...
    for lc in lcs
        ...
        @printf(
            "    α0=%+8.5f  dα0=%+8.5f  dτ0=%+8.5f  tf=%8.4f s  max|mu|!=1=%.3e\n",
            s[1],
            s[2],
            s[3],
            s[4] * T0(vr),
...
println(
    ...
    length(pp),
    "   ds range: [",
    round(minimum(dd); sigdigits=3),
    ", ",
    round(maximum(dd); sigdigits=3),
    "]",
)
