...
using Plots
using Serialization
using BifurcationKit: BifurcationKit                                  # resolve RecursiveArrayTools on deserialize
...
plt = plot(;
    layout=(1, 2),
    size=(1500, 780),
    plot_title="eight-circle layout — short cycle @ v_ref=2.48",
)
for (k, mode) in enumerate((:src, :rescaled))
    ...
    plot!(
        ...
        title=(
            if mode === :src
                "src proportions (true scale, circle tiny)"
            else
