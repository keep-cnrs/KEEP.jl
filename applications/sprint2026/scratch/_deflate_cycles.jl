...
function nearest_poincare(v)
    return (
        rows=deserialize(joinpath(@__DIR__, "bvp_cycles_poincare.jls"));
...
function warm_from_sampler(bvp, setup)
    ...
    interp =
        t -> (
            i=clamp(searchsortedfirst(tt, t), 1, length(tt));
            SA[U[1, i], U[2, i], U[3, i] / T0, U[4, i] / T0, Tn * T0]
        )
    x = BVP.generate_solution(bvp, interp)
    x[end] = 0.0
...
function bvp_from_orbit(bvp, u, t)
    tt = collect(t)
    tt = (tt .- tt[1]) ./ (tt[end] - tt[1])
    interp =
        s -> (
            i=clamp(searchsortedfirst(tt, s), 1, length(tt));
...
function avg_power(u, pv, setup)
    ...
    pw = [
        PM4.dynamics(
            SA[u[1, k], u[2, k], T0 * u[3, k], T0 * u[4, k], 0.0], get_fast_para(pv)
        )[5] for k in 1:size(u, 2)
    ]
...
function main()
    ...
    function add_root!(xr, tag)
        ...
        push!(roots, collect(xr))
        @printf("  %-10s root tf=%.4f s\n", tag, tfr)
        flush(stdout)
        return true
    end
    ...
    newton(xg) =
        try
            damped_newton(mkprob(xg), xg, pv; tol=1e-9, max_iter=100)[1]
        catch
            nothing
        end
    ...
    if isfile(archf) && abs(VREF - 2.47) < 1e-9
        ...
        add_root!(
            newton(bvp_from_orbit(bvp, Array(cyc.short.u), cyc.short.t)), "arch short"
        )
        ...
        for k in 2:length(roots)
            s = try
                BifurcationKit.solve(
                    ...
                    M1,
                    NewtonPar(;
                        tol=1e-8, verbose=false, linesearch=true, max_iterations=20
                    ),
                )
            catch
                nothing
            ...
            @printf(
                "  deflate root1 → from root%d: %s\n",
                k,
                if (s !== nothing && BifurcationKit.converged(s))
                    "recovered tf=$(round(raw_x(collect(s.u))[5], digits=4)) s"
                else
                    "no convergence"
                end
            )
    ...
    for (k, xr) in enumerate(roots)
        ...
        push!(
            ...
            (
                tf=raw_x(xr)[5],
                α0=o[2][1, 1],
                mu=monodromy_max(o[2], o[1], raw_x(xr)[5], pv),
                power=avg_power(o[2], pv, setup),
                cres=cycle_res(o[2]),
                tag=k,
            ),
        )
    ...
    for r in rows
        @printf(
            "%-4d %-10.4f %-+10.5f %-10.3e %-11.1f %-10.2e\n",
            r.tag,
            r.tf,
            r.α0,
            r.mu,
    ...
    return serialize(
        joinpath(@__DIR__, "deflated_cycles_V$(VREF)_M$(MSTAR).jls"),
