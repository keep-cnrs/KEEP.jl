...
using KEEP.LimitCycle: compute_limit_cycle, build_shooting, unpack_shooting, all_limit_cycles
...
function multipliers(s, vbp; tol=1e-11)
    ...
    pm = v -> copy(Array(integrate(SA[v[1], v[2], v[3], v[4], 0.0], T, vbp; tol=tol).u[end])[1:4])
...
pa = plot(bA.p, bA.tf; lw=1, label="collocation (brA)", xlabel="v_ref", ylabel="period tf (s)",
    title="limit-cycle branch, default params")
plot!(pa, bS.p, bS.tf; lw=1, alpha=0.5, label="shooting (brS)")

pb = plot([r.vr for r in rows_dn], [r.mu[2] for r in rows_dn]; label="from v_ref=9 down",
    yscale=:log10, xlabel="v_ref", ylabel="max nontrivial |mu|",
    title="ground-truth Floquet stability", ylims=(1e-4, 10), lw=2)
plot!(pb, [r.vr for r in rows_up], [r.mu[2] for r in rows_up]; label="from v_ref=9 up", lw=2)
...
for vr in (9.0, 5.0, 3.0)
    ...
    for lc in lcs
        ...
        @printf("v_ref=%5.2f  α0=%+7.4f  dα0=%+7.4f  dτ0=%+7.4f  T=%8.5f  P=%9.2f\n",
            vr, sh[1], sh[2], sh[3], sh[4], lc.u[end][5] / lc.t[end])
    end
end
...
for vr in (2.0, 3.0, 5.0, 7.0, 9.0, 12.0, 15.0)
    ...
    for q in ss
        ...
        @printf("   α=%+7.4f τ=%+7.4f  Re(eig)=%s  %s\n", q[1], q[2],
