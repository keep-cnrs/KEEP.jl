...
function main()
    ...
    sol = BifurcationKit.solve(
        prob, Newton(), NewtonPar(; tol=1e-10, verbose=false, linesearch=true)
    )
    ...
    t0 = time()
    br = continuation(
        prob, PALC(), optc; plot=false, verbosity=0, normC=norminf, bothside=true
    )
    p = collect(br.branch.param)
    tf = collect(br.branch.tf)
    ss = br.sol
    @printf(
        "branch: n=%d  sol=%d  p∈[%.4f,%.4f]  tf∈[%.3f,%.3f]  (%.1fs)\n",
        length(p),
        length(ss),
        minimum(p),
        maximum(p),
    ...
    for (lo, hi, name) in bands
        ...
        xp = try
            damped_newton(mkprob(x), x, pv; tol=1e-10, max_iter=60)[1]
        catch
            nothing
        end
        xp === nothing || (x = xp)
        ...
        U = Array(u)
        @printf(
            "  %-8s : node tf=%.4f → polished tf=%.4f s (crossing %.4f)  cycle res=%.2e\n",
            name,
            tfn,
            x[5],
            tfi,
    ...
    serialize(
        joinpath(@__DIR__, "three_cycles_shooting.jls"),
        (;
