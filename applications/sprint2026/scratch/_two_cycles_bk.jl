...
function g(u0, uT, p)
    return SA[uT[1] - u0[1], uT[3] - u0[3], uT[4] - u0[4], u0[2] - TAU0, uT[2] - TAU0 - 2π]
end
const STATE_SIZE = 5

const DISC = if MODE == "shooting"
    BVP.Shooting(40, ODE.Tsit5(), true)
else
...
model = make_model()
bvp = if MODE == "shooting"
    BVP.discretize(model, DISC; abstol=1e-12, reltol=1e-10)
else
...
if isempty(cand) || maximum(last, cand) / minimum(last, cand) < 1.3
    println(
        MODE,
        ": only ONE branch near v_ref=",
        VRT,
        " (did not pass the fold): ",
        if isempty(cand)
            "no saved point"
        else
    ...
    function orbit_of(x, p, tf)
        s = if MODE == "shooting"
            BVP.get_solution_bvp(
            bvp, @view(x[1:(n * DISC.M)]), BifurcationKit.setparam(prob, p)
        )
        else
    ...
    (t1, u1) = orbit_of(xshort, pv, tf1)
    (t2, u2) = orbit_of(xlong, pv, tf2)
    ...
    serialize(
        joinpath(@__DIR__, "two_cycles_$(MODE).jls"),
