...
const PARA_CACHE = Ref{
    Union{
        Nothing,Tuple{Float64,typeof(NamedTuple(build_vbpara(build_para(build_vbpara()))))}
    },
}(
    nothing
)
function get_fast_para(p)
    ...
    if c === nothing || c[1] != p.v_ref
        c = (p.v_ref, NamedTuple(build_vbpara(p)))
        PARA_CACHE[] = c
    end
    return c[2]
end
...
files = [
    f for f in ("two_cycles_shooting.jls", "two_cycles_collocation.jls") if
...
for f in files
    ...
    for tag in ("short", "long")
        ...
        X = reduce(
            hcat,
            [
                compute_OK(compute_Rτ([dd.u[1, k], dd.u[2, k]], vbp), vbp) for
                k in axes(dd.u, 2)
            ],
        )
