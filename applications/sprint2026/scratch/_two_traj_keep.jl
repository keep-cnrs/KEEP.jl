...
function main()
    ...
    for vr in (6.0, 4.0, 3.0)
        s = build_shooting(
            compute_limit_cycle(unpack_shooting(s)[1], vbp_at(vr); tf=120, tol=1e-10)
        )
    end
    ...
    for step in 1:400
        ...
        ds = min(ds * 1.3, 0.1)
        @printf(
            "step %3d  v_ref=%.5f  T=%.4f  tf=%.3f s  (ds=%.3f)\n",
            step,
            z[5],
            z[4],
            z[4] * T0(z[5]),
    ...
    @printf(
        ...
        sshort[4] * T0(vrt),
        rshort,
        join([@sprintf("%.4f", m) for m in multipliers(sshort, vrt)], "  ")
    )
    @printf(
        "  long : tf = %.4f s  res=%.1e  |mu| = %s\n",
        slong[4] * T0(vrt),
        rlong,
        join([@sprintf("%.4f", m) for m in multipliers(slong, vrt)], "  ")
    )
    ...
    plot!(
        ...
        t1,
        U1[1, :];
        label="short (tf=$(round(t1[end],digits=2))s)",
        xlabel="t (s)",
        ylabel="α (rad)",
        title="α",
    )
    ...
    plot!(
        p[2], t1, U1[3, :]; label="short", xlabel="t (s)", ylabel="dα (rad/s)", title="dα"
    )
    ...
    return println("saved BK_tests_0910_two_cycles.png")
end
