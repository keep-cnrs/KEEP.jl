...
with_logger(NullLogger()) do
    ...
    hline!([length(shootings) 2], c=[:black :red], alpha=.5, label=["n = #samples" "n = 2"])
...
annotate!(i, prop / 2 .+ .1, [f"P = \%.0f(Pavg) W" for Pavg in Pavg])
display(plot!())
...
function _limit_cycle_rootfind(shooting, vbp; sense=default_sense(shooting), tol=tol)
    ...
    shooting1 = compute_limit_cycle(u0, vbp; tol=.1)
