...
include("utils.jl"); using .ExampleUtils
...
@time _, _, fig = variation(:n_wind, 3:0.2:10, p0, syms, lower, upper, tol=1e-3)
display(fig)

_, _, fig = variation(:r, 40:80, p0, [:I_eq], [1000], [10000], tol=1e-5)
display(fig)

# Replace @test in optimize by if ... @warning ... end
_, _, fig = variation(:I_eq, 1000:100:10000, p0, [:r], [10], [100], tol=1e-5)
display(fig)

function average_power(p; tol)
    lc = compute_limit_cycle(p; sense=-, save_everystep=true, tol=tol)
...
function landscape_slices(var_syms, N_vals, p0, syms, lower, upper; kwargs...)
    ...
    objs = fill(NaN, length(N_vals), length(var_syms))
    @time with_logger(NullLogger()) do
