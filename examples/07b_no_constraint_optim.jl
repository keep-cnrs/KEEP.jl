...
function shoot(shoot_vars, vbp)
    ...
    sol = solve(ODEProblem(dynamics, u0, tf, vbp); reltol=tol, abstol=tol, save_everystep=true)
    α0, _, dα0, dτ0, _ = u0
...
function update_shoot_init_rootfind(optim_vars::ComponentArray{<:Dual}, vbp) update_shoot_init_rootfind(getfield.(optim_vars, :value), vbp) end

function update_shoot_init_compute_limit_cycle(optim_vars, vbp)
    vbp[keys(optim_vars)] .= optim_vars
    u0, tf, _ = unpack_shoot_vars(SHOOT_INIT)
    ode_prob = ODEProblem(dynamics, u0, 1000tf, vbp)
...
function update_shoot_init_compute_limit_cycle(optim_vars::ComponentArray{<:Dual}, vbp) update_shoot_init_compute_limit_cycle(getfield.(optim_vars, :value), vbp) end
...
function update_shoot_init_integration(optim_vars, vbp)
    ...
    vbp[keys(optim_vars)] .= optim_vars
    u0, tf, _ = unpack_shoot_vars(SHOOT_INIT)
    ode_prob = ODEProblem(dynamics, u0, 100tf, vbp)
...
function update_shoot_init_integration(optim_vars::ComponentArray{<:Dual}, vbp) update_shoot_init_integration(getfield.(optim_vars, :value), vbp) end
...
function obj(optim_vars::ComponentArray, vbp)
    ...
    vbp[keys(optim_vars)] .= optim_vars
    u0, tf, _ = unpack_shoot_vars(SHOOT_INIT)
    ode_problem = ODEProblem(dynamics, u0, 2tf, vbp)
    ...
    if SHOOT_INIT[3] > 0 @show "Le kite tourne dans le mauvais sens" end
...
function obj_one_integration(dτ_sign::Union{typeof(+), typeof(-)})
    function obj_one_integration(optim_vars::ComponentArray, vbp)
        ...
        cb = CallbackSet(cb1, cb2)
        ode_prob = ODEProblem(dynamics, u0, 1e4, vbp)
        sol = solve(ode_prob; callback=cb, save_everystep=false, save_start=false, save_end=false, reltol=tol, abstol=tol)
        @show 3
...
begin
para = para1; vbp0 = build_vbpara(para); L, M, T = lmt(vbp0)
α0, dα0, dτ0 = 0, 0, 0
tol = 1e-9
...
ode_prob = ODEProblem(dynamics, unpack_shoot_vars(SHOOT_INIT)[1], 1000*char_time / T, vbp)
limit_cycle = compute_limit_cycle(ode_prob; tol=tol)
...
function benchmark_obj(obj, optim_vars, vbp, N)
    ...
    TOL .*= 1e6
    for f in (obj, Dobj, DDobj) @time f(optim_vars, vbp) end
    TOL ./= 1e6

    for f in (obj, Dobj, DDobj) @time for i in 1:N f(optim_vars, vbp) end
    end
end
