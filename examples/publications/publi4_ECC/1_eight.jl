...
annotate!(sum(φ.([0, τ_φ])) / 2, sum(θ.([τ_φ, τ_φ])) / 2 - .1, ("\$Δφ\$", ANNOTATIONFONTSIZE, delta_col, :top))

τ_θ = 225.0
# plot!(φ.([τ_θ, τ_θ]), θ.([0, τ_θ]), arrow=false, color=delta_col, label="")
plot!(φ.([τ_θ, τ_θ]), θ.([τ_θ, 0]), arrow=false, color=delta_col, label="")
annotate!(sum(φ.([τ_θ, τ_θ])) / 2 + .05, sum(θ.([τ_θ, 0])) / 2, ("\$Δθ\$", ANNOTATIONFONTSIZE, delta_col, :left))
...
annotate!(dx-.05, -dy-0.25, ("\$\\dot{\\tau} > 0\$", ANNOTATIONFONTSIZE, :center, palette(:auto)[up_col]))
annotate!(-dx+.05, dy+0.25, ("\$\\dot{\\tau} < 0\$", ANNOTATIONFONTSIZE, :center, palette(:auto)[down_col]))

display(plot!())

savefig(plot!(), "test/publi4_ECC/figs/eight_plot.pdf")
