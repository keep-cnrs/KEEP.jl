...
cm.hidespines!(ax)
cm.poly!([Makie.Rect(-π + i*π/2, -π, π/2, 2π) for i in 0:3], color=[:red, :green, :green, :red], alpha=.1)
col_α = PALETTE[4] # :steelblue2
col_τ = PALETTE[3] # :chartreuse2

cm.streamplot!(ax, cm.Point2 ∘ f, (-π, π), (-π, π); colormap=:binary, density=1, color=p -> norm(p), arrow_size=5, linewidth=.4, gridsize=(20, 20), stepsize=.1, maxsteps=10)
...
s = cm.scatter!(ax, invert(ss)...; markersize=10, color=max_eigvals, marker=:circle, strokewidth=.5, colormap=Makie.Reverse(:imola))

line_α = cm.LineElement(color=col_α)
line_τ = cm.LineElement(color=col_τ)
L"\max_i \mathrm{Re}(\lambda_i)"
cb = cm.Colorbar(fig[1, 2], s, label=L"\max_i \ \mathrm{Re}(\lambda_i)", ticklabelfont = "CMU Serif", height=cm.Relative(.9))
...
# cm.save("test/publi4_ECC/figs/equilibria.png", fig, px_per_unit=PT_PER_INCH)
cm.save("test/publi4_ECC/figs/equilibria.pdf", fig, px_per_unit=PT_PER_INCH * PT)
