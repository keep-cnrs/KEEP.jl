...
display(plot!())
savefig("test/publi4_ECC/figs/limit_cycle_convergence.pdf")
...
@_ plot!(invert(yz_split(lc_p, n_splits))..., arrows=true, label="\$\\dot{\\tau} > 0\$")
@_ plot!(invert(yz_split(lc_m, n_splits))..., arrows=true, label="\$\\dot{\\tau} < 0\$")
plot!([0], [13])
lens!([-11, -10], [8, 9], inset = (1, bbox(.3, 0, .3, .4)))
...
savefig(plot!(), "test/publi4_ECC/figs/yz_limit_cycles.pdf")
