...
u0_opt = SA[solution[1], LC.TAU0, solution[2], solution[3], 0]
...
plot(size= plot_size(4/3), xticks=(0:.25:1, ["0", "", "", "", "1"]), xlabel="Normalized time", yticks=0:5:20, ylim=(0, 20), ylabel="Power (kW)", legend=:outerbottom, bottom_margin=-8Plots.mm)
...
annotate!(.28, avg_opt + 2, Plots.text("\$$(round(avg_opt, sigdigits=2))\$ kW", ANNOTATIONFONTSIZE, c_opt, :left, rotation=0, family=FONT))
...
savefig("test/publi4_ECC/figs/optimized_power.pdf")