...
plot!(f2, wind_range, max_tension_vec ./ 1000, fillrange=min_tension_vec ./ 1000, color=1, fillcolor=1, label="Tension range (kN)", fillalpha=.1)
...
if legend == :annotate
    ...
    annotate!(f3, wind_range[end]-.2, 0.7, ("Arm inertia", TICKFONTSIZE, :right, palette(:auto)[2]))
...
savefig(fig, "test/publi4_ECC/figs/parametrized_optimization.pdf")
savefig(f1, "test/publi4_ECC/figs/parametrized_optimization_power.pdf")
savefig(f2, "test/publi4_ECC/figs/parametrized_optimization_tension.pdf")
savefig(f3, "test/publi4_ECC/figs/parametrized_optimization_params.pdf")
...
plot!(wind_range, [1 / (theta[1]-45) + .2 for theta in theta_vec], label="line length")
plot!(wind_range, [(125 / (theta[2] - 3270) + .2)/1.2 for theta in theta_vec], label="arm inertia")
plot!([0], [0])
...
my_defaults()
##
plot(r_range[inds], @. (Is[inds] / 8000 + .3)); plot!([0], [0])