...
function steadiest_τ(α_func, τ_init, p; tol=DEFAULT_TOLERANCE)
    ...
    τ_opt = only(
        solve(NonlinearLeastSquaresProblem(opt_func, [τ_init], p); abstol=tol, reltol=tol).u
    )
...
function steadiest_aligned_state(
    τ_init, p, orientation::Val{:same}=Val(:same); tol=DEFAULT_TOLERANCE
)
    return steadiest_τ(τ -> aligned_α(τ, p), τ_init, p; tol=tol)
end

function steadiest_aligned_state(
    τ_init, p, orientation::Val{:opposite}; tol=DEFAULT_TOLERANCE
)
...
function steady_state(α_init::T1, τ_init::T2, p; tol=DEFAULT_TOLERANCE) where {T1,T2}
    ...
    root =
        solve(NonlinearProblem(ddq_partial, T[α_init, τ_init], p); abstol=tol, reltol=tol).u
...
function steady_state(τ_init, p; orientation=Val(:same), tol=DEFAULT_TOLERANCE)
    ...
    return steady_state(α_aligned_, τ_aligned, p; tol=tol)
end
...
function all_steady_states_halton(
    p; αmin=(-π), αmax=π, τmin=0, τmax=π, N=100, tol=DEFAULT_TOLERANCE
)
