...
plot!(
    sol10.t,
    [norm(PM10.manifold_residuals!(similar(u0, 6), u, p)) for u in sol10.u];
    label="Without",
)
plot!(
    sol10_cb.t,
    [norm(PM10.manifold_residuals!(similar(u0, 6), u, p)) for u in sol10_cb.u];
    label="With",
)
display(plot!())
...
_, _, T = lmt(p)
sol4_normed = PM4.integrate(
    SA[0.0, τ0, 0.0, dτ0 * T, 0.0], tf / T, vbp_normed; save_everystep=true
)
...
if false
    fps = 30
    anim = VIS.animate_trajectory_4D(
        @set sol4.prob.p.r = 6;
        tspan=(0, tf_small),
        fps=fps,
        end_camera=(30, 30),
