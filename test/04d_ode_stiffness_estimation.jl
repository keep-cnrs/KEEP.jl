...
orders =
    -log.(errors[(1 + 2skip):end] ./ errors[1:(end - 2skip)]) ./
    log.(Ns_error[(1 + 2skip):end] ./ Ns_error[1:(end - 2skip)])
...
plot(
    Ns_order, orders; label="Order", marker=:circle, xaxis=:log, minorgrid=true, xlabel="dt"
)
