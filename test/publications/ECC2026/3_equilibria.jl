...
function main()
    ...
    function max_eigval(q)
        return maximum(
            real.(eigvals(collect(jacobian(u -> PM4.dynamics(u, p, 0), SA[q..., 0, 0, 0]))))
        )
    end
    ...
    Makie.set_theme!(Makie.Theme(; font="CMU Serif", fontsize=2.2 * TICKFONTSIZE))
    ...
    cm.poly!(
        [Makie.Rect(-π + i * π / 2, -π, π / 2, 2π) for i in 0:3];
    ...
    cb = cm.Colorbar(
        fig[1, 2],
        s;
        label=L"\max_i \ \mathrm{Re}(\lambda_i)",
        ticklabelfont="CMU Serif",
        height=cm.Relative(0.9),
    )
    ...
    cm.save(
        "test/publications/ECC2026/figs/equilibria.pdf", fig; px_per_unit=PT_PER_INCH * PT
    )
