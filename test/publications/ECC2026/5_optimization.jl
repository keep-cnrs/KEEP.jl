...
function main()
    ...
    results_rows = [
        f"    {v.sym} ({v.unit}) & {v.guess:.1f} & {v.sol:.1f} \\\\ " for v in data
    ]
    ...
    push!(
        results_rows,
        f"    Average power (\\si{{\\kilo\\watt}}) & {initial_guess[5]/1000:.2f} & {solution[5]/1000:.2f} \\\\ ",
    )
    ...
    push!(
        results_rows,
        raw"    Norm of residual & {" * res_guess * raw"} & {" * res_sol * raw"} \\ ",
    )
...
\end{table}"""
    ...
    println(
        join(
            ["%% BEGIN TABLE 2", table_params_str, table_opt_str, "%% END TABLE 2"], "\n\n"
        ),
    )
    ...
    return savefig(plot!(), joinpath(outpath, "optimized_power.pdf"))
end
