...
Arm braking & $b$ [\si{\newton\metre\per\radian\per\second}] & " *
        ...
        f"{upper[2]:.1f}" *
        raw" \\
Arm inertia & $I$ [\si{\kilogram\metre\squared}] & " *
        f"{lower[3]:.1f}" *
        " & " *
        f"{upper[3]:.1f}" *
        raw" \\
...
\end{table}"
    ...
    table_opt_str *=
        "\n    " *
        join(
            [raw"$\ell$", f"{initial_guess[end-2]:.1f}", f"{solution[end-2]:.1f}"], " & "
        ) *
        raw" \\ "

    table_opt_str *=
        "\n    " *
        join([raw"$b$", f"{initial_guess[end-1]:.1f}", f"{solution[end-1]:.1f}"], " & ") *
        raw" \\ "

    table_opt_str *=
        "\n    " *
        join([raw"$I$", f"{initial_guess[end]:.1f}", f"{solution[end]:.1f}"], " & ") *
        raw" \\ "
    ...
    table_opt_str *=
        "\n    " *
        join(
            [raw"Average power (W)", f"{initial_guess[5]:.1f}", f"{solution[5]:.1f}"], " & "
        ) *
        raw" \\ "
    ...
    println(
        join(
            ["% ## BEGIN TABLE", table_params_str, table_opt_str, "% ## END TABLE"], "\n\n"
        ),
    )
