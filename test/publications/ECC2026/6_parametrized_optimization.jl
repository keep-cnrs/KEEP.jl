...
function range_optim(param, values; syms, p0, sense, tol)
    stats_no_opt_vec, stats_opt_vec, theta_vec = invert(
        @showprogress y = [
...
function main()
    ...
    stats_no_opt_vec, stats_opt_vec, theta_vec = range_optim(
        :v_ref, wind_range; syms=syms, p0=p_ref, sense=sense, tol=tol
    )

    ## Plot
    my_defaults()
    power_vec, min_tension_vec, max_tension_vec, tf_vec, ode_plot_vec = invert(
        stats_opt_vec
    )
    ...
    plot(
        wind_range,
        [sqrt.(max_tension_vec ./ 1000) ./ 6, sqrt.(min_tension_vec ./ 1000) ./ 4];
        c=:blue,
        label="tension",
    )
    ...
    plot!(
        wind_range,
        [theta[3] / 7000 for theta in theta_vec];
        label="arm braking coefficient",
    )

    plot!(
        wind_range, [1 / (theta[1] - 45) + 0.2 for theta in theta_vec]; label="line length"
    )
    plot!(
        wind_range,
        [(125 / (theta[2] - 3270) + 0.2) / 1.2 for theta in theta_vec];
        label="arm inertia",
    )
    plot!([0], [0])
    ...
    r_range = 10:5:100
    _, _, theta_vec_ = range_optim(
        :r, r_range; syms=[:I_eq], p0=p_ref, sense=sense, tol=tol
    )
